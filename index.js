...
function processLength(list, callback) {
  ...
  return callback(list[list.length i])
}
...
function stringSmash(strings) {
  let together = strings.reduce((str, current) => {
    return str + current;
  });
  return together;
}
...
function firstNamesAllCaps(runners) {
  const updateRunners =runners.map(function(item){
 ...
 * 
 * 2. Which of the two uses a closure? How can you tell? function 2 is a clouser. It's a clouser because the count variable is passing down into counter2 function.
 * 
